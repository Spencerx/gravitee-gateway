...
public class FailoverInvoker extends EndpointInvoker implements InitializingBean {
    ...
    @Override
    public Request invoke(ExecutionContext context, ReadStream<Buffer> stream, Handler<ProxyConnection> connectionHandler) {
        final Request failoverServerRequest = new FailoverRequest(context.request());

        ((SimpleExecutionContext)context).request(failoverServerRequest);
